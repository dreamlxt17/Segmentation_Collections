...
import numpy as np
from .simpleLocal import NonLocal3, NonLocal1, NonLocal2, NonLocal4, NonLocal5
#from utils import Separable1
...
class SpacingUnet(nn.Module):
    def __init__(self, num_classes=2, nonLocalBlock=NonLocal1, **kwargs):
        ...
        self.conv3_1 = nonLocalBlock(in_channels=64, sub_sample=True, spacing_thresh=1.5)
        self.conv3_2 = ConvBlock(64, 128, None)
        self.pool3 = nn.MaxPool3d(kernel_size=(2, 2, 1))
        self.conv4_2 = ConvBlock(128, 128, None)
        self.conv4_1 = nonLocalBlock(in_channels=128, sub_sample=True, spacing_thresh=3)
        self.pool4 = nn.MaxPool3d(kernel_size=(2, 2, 1))
        self.conv5_2 = ConvBlock(128, 128, None)
        self.conv5_1 = nonLocalBlock(in_channels=128, sub_sample=True, spacing_thresh=6)
    ...
    def forward(self, x, spacings):
        # insert another non-local block at layer3, layer4 and layer5
        conv1 = self.conv1(x)
        pool1 = self.pool1(conv1)
        conv2 = self.conv2(pool1)
        pool2 = self.pool2(conv2)
        conv3_1 = self.conv3_1(pool2, spacings)
        conv3_2 = self.conv3_2(conv3_1)
        pool3 = self.pool3(conv3_2)
        conv4_1 = self.conv4_1(pool3, spacings)
        conv4_2 = self.conv4_2(conv4_1)
        pool4 = self.pool4(conv4_2)
        conv5_1 = self.conv5_1(pool4, spacings)
...
class SpacingUnet1(SpacingUnet):
    def __init__(self, num_classes=2, nonLocalBlock=NonLocal1, **kwargs):
        super(SpacingUnet1, self).__init__(
            num_classes=num_classes, nonLocalBlock=nonLocalBlock
        )


class SpacingUnet4(SpacingUnet):
    def __init__(self, num_classes=2, nonLocalBlock=NonLocal2, **kwargs):
        super(SpacingUnet4, self).__init__(
            num_classes=num_classes, nonLocalBlock=nonLocalBlock
        )


class SpacingUnet5(SpacingUnet):
    def __init__(self, num_classes=2, nonLocalBlock=NonLocal4, **kwargs):
        super(SpacingUnet5, self).__init__(
            num_classes=num_classes, nonLocalBlock=nonLocalBlock
        )


class SpacingUnet6(SpacingUnet):
    def __init__(self, num_classes=2, nonLocalBlock=NonLocal5, **kwargs):
        super(SpacingUnet6, self).__init__(
            num_classes=num_classes, nonLocalBlock=nonLocalBlock
        )


#class SeparableUnet1(SpacingUnet):
#    def __init__(self, num_classes=2, nonLocalBlock=Separable1, **kwargs):
#        super(SeparableUnet1, self).__init__(
#            num_classes=num_classes, nonLocalBlock=nonLocalBlock
#        )
